...
from pac_bound_tools import validate_inputs, xi, solve_kl_sup, maximize_c_bound_under_constraints
from math import log

def pac_bound_two_prime(empirical_gibbs_risk, empirical_disagreement, m, KLQP, delta=0.05):
    """ PAC Bound TWO PRIME of Germain, Lacasse, Laviolette, Marchand and Roy (JMLR, 2014)
...
if __name__ == '__main__':
    ...
    if argc < 3 :
        print('-'*100)
        print('Usage: pac_bound_2p.py empirical_gibbs_risk empirical_disagreement [m] [KLQP] [delta]')
        print('-'*100)
        print(pac_bound_two_prime.func_doc)
    else:
        ...
        bound = pac_bound_two_prime(**arg_dict)
